...
class Item:
    ...
    def calculate_worry(self, arg1, operator, arg2):
        if arg1 == 'old':
            a_1 = self.value
        else:
            a_1 = int(arg1)
    ...
    for idx in range(0, 10000):
        for m_idx in MONKEYS:
            ...
            for item in monkey_items:
                ...
                item.calculate_worry(*monkey.operations) #, mod=int(m_idx)+1)
                item.value = int(item.value - (item.value / 2))
...
if __name__ == "__main__":
    ...
    for i in range(1,2):
        print('Processing with value of {}'.format(i))
        try:
            process_loop(i)
        except Exception as excp:
            print('excp was :{0}'.format(excp))
            continue
